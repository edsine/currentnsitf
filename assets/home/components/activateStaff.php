...
require("PHPMailer_5.2.0/class.pop3.php");
  ...
  $mail = new PHPMailer();
            ...
            $cert = $_GET['id'];
             ...
             $staffEmail = $query->getRow("select staffId, staff_email, alternative_email  from staff_tb where staffId =  $cert ");
     
     $cemail =  $staffEmail['staff_email'];
     
       $cemail =  $staffEmail['alternative_email'];
      ...
      $update = $query->updateRow("update staff_tb set roles=?, branchId=?, active_staus=?, security_key=? where staffId = ?", ["$role","$branch","1", "$password", "$cert"]);
