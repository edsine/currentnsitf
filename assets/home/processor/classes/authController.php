...
$conn = new mysqli('192.3.204.226z', 'navsang', '")7hGTT22z0r:Oc', 'navsang_db');
...
if (isset($_POST['signup-btn'])) {
    ...
    if (empty($_POST['address'])) {
     ...
     if (empty($_POST['as_id'])) {
        $errors['as_id'] = 'Password required';
    }
   ...
   $token = bin2hex(random_bytes(50));// generate unique token
    ...
    if ( $check) {
     ...
     if (count($errors) === 0) {
$result = $query->insert("INSERT INTO artisan_tb( f_email, auth_token,password, ip_address)values('$email', '$token', '$password','$ip')");
