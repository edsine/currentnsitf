   ...
   // if(in_array($fileType, $allowTypes)){
      ...
      //  if(move_uploaded_file($_FILES["passport"]["tmp_name"], $targetFilePath)){
                 ...
                 $pin = rand(100000,999999);
                  ...
                  $type = 2;
                $insert_query = "INSERT INTO `c_artisan`(`fname`, `phone`, `email`,`gendar`, `state`,`experience`, industry_id, `service_id`, `pstate`, `plocal`, `w-address`, `nin`, `password`, `datedAt`) VALUES(:name,:phone,:email,:gendar,:state,:experience,:ids_id,:service_id, :pstate, :plocal, :waddress, :nin, :password, :dated)";

                $insert_stmt = $conn->prepare($insert_query);

                // DATA BINDING
                  $insert_stmt->bindValue(':name', $fname,PDO::PARAM_STR);
               ...
               $insert_stmt->bindValue(':state', $state,PDO::PARAM_STR);
                ...
                $insert_stmt->bindValue(':experience', $exp,PDO::PARAM_STR);
                $insert_stmt->bindValue(':ids_id', $inds,PDO::PARAM_STR);
                 $insert_stmt->bindValue(':service_id', $service,PDO::PARAM_STR);
                   $insert_stmt->bindValue(':pstate', $state,PDO::PARAM_STR);
                     $insert_stmt->bindValue(':plocal', $plocal,PDO::PARAM_STR);
                 $insert_stmt->bindValue(':waddress', $address,PDO::PARAM_STR);
                   $insert_stmt->bindValue(':nin', $nin,PDO::PARAM_STR);
                $insert_stmt->bindValue(':password', password_hash($password, PASSWORD_DEFAULT),PDO::PARAM_STR);
                 
                 
                  $insert_stmt->bindValue(':dated', $today,PDO::PARAM_STR);
                  
                  //  $insert_stmt->bindValue(':passport', $fileName,PDO::PARAM_STR);
                
            
               $result = $insert_stmt->execute();
                $artisan = $conn->lastInsertId();
                ...
                if($result){
                   ...
                   //  $insert_query = "INSERT INTO `service_request`(`artisanId`,`clientId`,`ids_id`, serviceId) VALUES(:artisan,:client,:ids,:service)";
//
              //   $insert_stmt = $conn->prepare($insert_query);

             // DATA BINDING
              //  $insert_stmt->bindValue(':artisan',$artisan,PDO::PARAM_INT);
             //   $insert_stmt->bindValue(':client',$state,PDO::PARAM_INT);
            //    $insert_stmt->bindValue(':ids', $inds,PDO::PARAM_INT);
           //     $insert_stmt->bindValue(':service',$service,PDO::PARAM_INT);
          //      $insert_stmt->execute();
