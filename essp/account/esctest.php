...
$paymentDetails = $query->getRow("select company_name, ecs_number, c_email, desk_phone from employer_tb  where employer_id = $employer"); 
 
 
 $checkYear = $query->getRow(" SELECT * FROM transactions WHERE employerId =$employer and payment_type = 4 and YEAR(payment_time)=$year"); 
...
$ecs = $query->getRow("select sum(monthly_remuneration) as remuSum from employees  where employer_id = $employer"); 

$empNum = $query->getRow("select count(*) as rem from employees  where employer_id = $employer"); 
...
$msg='<script type="text/javascript">alert("Your ECS  payment is succesful");</script>';
...
>
  ...
  <body>
   ...
   <div class="container-xxl flex-grow-1 container-p-y">
                    ...
                    <div class="d-flex align-items-end row">
                      <div class="col-sm-9">
                        <div class="card-body" style="">
                          <h5 class="card-title " style="font-size:20px;font-weight:bold; color:black;"> ECS Payment  </h5>
                          <?php if(!$checkYear){   ?>
                          <p class="mb-4" style="font-size:18px;">
                             Your ecs payment for <?php echo $emp ?> employees is  <span class="fw-bold"><?php echo $ecs ?> </span> 
                          </p>
            ...
            <input type="hidden" id="js-amount" value="<?php echo $ecs ?>" name="amount" class="field-long"/>
...
$.ajax({
         ...
         //  window.location.href = encodeURIComponent("testR.php?pf="+datas1.RRR) 
           ...
           window.location.href="saveRRR.php?rrr="+datas1.RRR +"&type=4"
         ...
         <?php }else{ ?>
          ...
          <p class="mb-4" style="font-size:18px;">
                             Your ecs payment for the year <?php echo $year ?> of <?php echo $emp ?> employees with the amount <span class="fw-bold"><?php echo '₦'.$ecs ?> is successful</span> 
                             <p>You will receive an email notification after approval from the management</p>
                          </p>
                    ...
                    <div class="col-lg-6 col-md-12 col-6 mb-4">
                      <div class="card">
                        <div class="card-body">
                          ...
                          <span>ECS</span>
                          <h3 class="card-title text-nowrap mb-1" style="font-size:12px;"><?php echo '₦'.$ecs ?></h3>
                          <small class="text-success fw-semibold"><i class="bx bx-up-arrow-alt"></i> +0.0%</small>
                        </div>
                      </div>
                    </div>
