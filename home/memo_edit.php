...
require_once '../classes/manage.php'; // this is the databse ....fmpaul1
$query = new Manage();

// a variable called memo_id 
 // i need id 
$theid= $query->getRows("select memo_id from memo_data");

$employees = $query->getRows("select * from memo_data WHERE memo_id='$theid' ");
...
<?php include("components/sidebar.php"); ?>
  ...
  <option value="all" <?php if($rows['destination']=="all"){echo "selected";} ?>>All Branch</option>
  <?php
                                                                                        require_once "db.php";
...
?>
  ...
  <textarea class="form-control" name="content" id="textAreaExample1" placeholder="Memo: Write here" rows="4"></textarea>
                       ...
                       <br>
                        <button type="submit" name="update" class="btn btn-primary">Update Braodcast</button>
                      </form>
                    </div>
                  </div>
                </div>
